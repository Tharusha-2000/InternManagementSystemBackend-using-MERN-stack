...
exports.updatedIntern = async (req, res) => {
  try {
    ...
    const updatedIntern = await User.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!updatedIntern) {
...
exports.getTask = async (req, res) => {
  ...
  if (req.data.role !== "intern") {
    return res
      .status(401)
      .send({ error: "You are not authorized to set this data" });
  }
  Task.find({
    _userId: id,
  })
...
exports.createTask = async (req, res) => {
  ...
  if (req.data.role !== "intern") {
    return res
      .status(401)
      .json({ error: "You are not authorized to set this data" });
  }
...
exports.getTaskVarify = async (req, res) => {
  ...
  try {
    ...
    const varifytask = await Task.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!varifytask) {
      return res.status(404).json({ message: "Task not found" });
...
exports.storeMentorScoresById = async (req, res) => {
  ...
  try {
    ...
    evaluationFormDetails.job_performance_scores_mentor =
      jobPerformanceScoresMentor;
    evaluationFormDetails.overall_performance_mentor =
...
// view evaluationformd by id mentor and evaluator
